...
from backend.user_service.profile import get_users_private_info_lambda
...
def test_no_jwt_token_return_401(create_users_table):
    ...
    # Act
    result = get_users_private_info_lambda.lambda_handler(event, "")
...
def test_when_successful_retrieval_of_data_return_200(create_users_table, create_jwt_secret, setup_env_variables):
    # Arrange
    ...
    assert response["info"]['age'] == 30
    assert response["info"]['phone_number'] is None