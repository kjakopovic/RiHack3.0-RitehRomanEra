...
from backend.user_service.profile import update_users_private_info_lambda
...
def test_no_jwt_token_return_401(create_users_table):
    ...
    # Act
    result = update_users_private_info_lambda.lambda_handler(event, "")
...
def test_when_nothing_passed_to_update_stays_the_same_return_200(create_users_table, create_jwt_secret, setup_env_variables):
    ...
    assert "message" in response
    assert response["message"] == "Your private info has been updated successfully."
    assert expected_user['Item']['age'] == 30
...
def test_when_items_passed_to_update_return_200(create_users_table, create_jwt_secret, setup_env_variables):
    ...
    assert response["message"] == "Your private info has been updated successfully."
    assert expected_user['Item']['age'] == 30
    assert expected_user['Item']['phone_number'] == '0997771717'
